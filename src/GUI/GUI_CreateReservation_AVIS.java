...
public class GUI_CreateReservation_AVIS
{
	...
	public static void main(String[] args) throws IOException, Exception, FileNotFoundException {
		...
		for (int k = 1; k <= 65; k++)
		{
			...
			ReadWriteExcel rwe = new ReadWriteExcel("C:\\Downloads\\Selenium\\AVIS\\TestData\\AVIS_GUITestData_CreateReservation.xlsx");
			...
			if (Execute.equals("Y"))
			{
				...
				if (rwe.getCellData("Avis_GUI", k, 19).isEmpty()) {
					...
					avis.clickRateshopSearchBtn(driver);
					WebElement ratetabel = driver
							.findElement(By.xpath("//*[@id='resRateLookupDlg:rateLookupForm:discountAwdRates']/table"));
					ArrayList<WebElement> radio1 = (ArrayList<WebElement>) ratetabel
							.findElements(By.xpath("//input[@name='radioRate' and @type='radio']"));
					try {
